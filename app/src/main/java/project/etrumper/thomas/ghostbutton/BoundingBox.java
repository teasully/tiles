 ...
 */
public class BoundingBox extends Attribute {

    float[] radii, modelMatrix;
	String objectName;
	boolean draw;

    BoundingBox(float[] modelMatrix, float[] radii, String objectName) {
        super(0, "BoundingBox", AttributeType.BOUNDINGBOX);
    ...
    BoundingBox(float[] modelMatrix, float[] lowValuesCorner, float[] highValuesCorner, String objectName) {
        super(0, "BoundingBox", AttributeType.BOUNDINGBOX);
        this.modelMatrix = modelMatrix;
        Matrix.translateM(this.modelMatrix, 0,
                (lowValuesCorner[0] + highValuesCorner[0]) * 0.5f,
                (lowValuesCorner[1] + highValuesCorner[1]) * 0.5f,
                (lowValuesCorner[2] + highValuesCorner[2]) * 0.5f);
        radii = new float[]{(highValuesCorner[0] - lowValuesCorner[0]) * 0.5f,
                (highValuesCorner[1] - lowValuesCorner[1]) * 0.5f,
                (highValuesCorner[2] - lowValuesCorner[2]) * 0.5f};
        this.objectName = objectName;
		this.draw = false;
    }
    ...
    protected float[] update(float[] data){
        if(data == null || data.length != 16){
            LOGE("Trying to update without sending in model matrix");
            return new float[]{};
        }
        for(int i = 0; i < 16; i++){
            this.modelMatrix[i] = data[i];
        }
        return new float[]{};
    }

    public static boolean IsLineInBox(float[] P1, float[] P2, BoundingBox box) {
        // Put line in box space
        ///CMatrix MInv = m_M.InvertSimple();
        float[] MInv = new float[16];
        Matrix.invertM(MInv, 0, box.modelMatrix, 0);
        float[] LB1 = VectorMath.transform(MInv, P1);
        float[] LB2 = VectorMath.transform(MInv, P2);
        //CVec3 LB1 = MInv * L1;
        //CVec3 LB2 = MInv * L2;

        // Get line midpoint and extent
        //CVec3 LMid = (LB1 + LB2) * 0.5f;
        float[] LMid = VectorMath.multiplyVectorBy(VectorMath.addVectors(LB1, LB2), 0.5f);
        //CVec3 L = (LB1 - LMid);
        float[] L = VectorMath.subtractVectors(LB1, LMid);
        //CVec3 LExt = CVec3( fabs(L.x), fabs(L.y), fabs(L.z) );
        float[] LExt = new float[]{Math.abs(L[0]), Math.abs(L[1]), Math.abs(L[2])};

        // Use Separating Axis Test
        // Separation vector from box center to line center is LMid, since the line is in box space
        //if ( fabs( LMid.x ) > m_Extent.x + LExt.x ) return false;
        if (Math.abs(LMid[0]) > box.radii[0] + LExt[0]) return false;
        //if ( fabs( LMid.y ) > m_Extent.y + LExt.y ) return false;
        if (Math.abs(LMid[1]) > box.radii[1] + LExt[1]) return false;
        //if ( fabs( LMid.z ) > m_Extent.z + LExt.z ) return false;
        if (Math.abs(LMid[2]) > box.radii[2] + LExt[2]) return false;
        // Crossproducts of line and each axis
        //if ( fabs( LMid.y * L.z - LMid.z * L.y)  >  (m_Extent.y * LExt.z + m_Extent.z * LExt.y) ) return false;
        if (Math.abs(LMid[1] * L[2] - LMid[2] * L[1]) > (box.radii[1] * LExt[2] + box.radii[2] * LExt[1]))
            return false;
        //if ( fabs( LMid.x * L.z - LMid.z * L.x)  >  (m_Extent.x * LExt.z + m_Extent.z * LExt.x) ) return false;
        if (Math.abs(LMid[0] * L[2] - LMid[2] * L[0]) > (box.radii[0] * LExt[2] + box.radii[2] * LExt[0]))
            return false;
        //if ( fabs( LMid.x * L.y - LMid.y * L.x)  >  (m_Extent.x * LExt.y + m_Extent.y * LExt.x) ) return false;
        if (Math.abs(LMid[0] * L[1] - LMid[1] * L[0]) > (box.radii[0] * LExt[1] + box.radii[1] * LExt[0]))
            return false;
